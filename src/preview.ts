(function () {
  ...
  async function waitForMermaid(): Promise<MermaidApi | undefined> {
  ...
  function openDiagramDialog(panel: HTMLElement): void {
  ...
  function createDialogButton(text: string, label: string, onClick: () => void): HTMLButtonElement {
  ...
  function closeDiagramDialog(): void {
    if (!activeDialog) {
      return;
    }
  ...
  function rewriteSvgIds(svg: SVGSVGElement, prefix: string): void {
  ...
  function getPanelViewport(panel: HTMLElement): HTMLElement {
    const viewport = panel.querySelector<HTMLElement>('.diagram-viewport');
  ...
  function getExpandButton(panel: HTMLElement): HTMLButtonElement {
    const button = panel.querySelector<HTMLButtonElement>('.diagram-expand-button');
